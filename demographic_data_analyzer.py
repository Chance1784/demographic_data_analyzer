...
def calculate_demographic_data(print_data=True):
    ...
    # Filter for higher and lower education
    higher_education = df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])
    lower_education = ~higher_education

    # Percentage of people with higher education who earn >50K
    higher_education_rich = round((df[higher_education & (df['salary'] == '>50K')].shape[0] / df[higher_education].shape[0]) * 100, 1)

    # Percentage of people with lower education who earn >50K
    ...
    country_salary_percentages = df[df['salary'] == '>50K'].groupby('native-country').size() / df.groupby('native-country').size() * 100
    highest_earning_country = country_salary_percentages.idxmax()
    highest_earning_country_percentage = round(country_salary_percentages.max(), 1)
